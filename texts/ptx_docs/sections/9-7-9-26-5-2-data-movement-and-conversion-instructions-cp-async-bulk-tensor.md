###### 9.7.9.26.5.2. Data Movement and Conversion Instructions: `cp.async.bulk.tensor`
...
The operand `tensorMap` is the generic address of the opaque tensor-map object which resides in `.param` space or `.const` space or `.global` space. The operand `tensorMap` specifies the properties of the tensor copy operation, as described in [Tensor-map](#tensor-tensormap). The `tensorMap` is accessed in tensormap proxy. Refer to the *CUDA programming guide* for creating the tensor-map objects on the host side.
...
| .completion-mechanism | `.dst` | `.src` | Completion mechanism | Completion mechanism |
| --- | --- | --- | --- | --- |
...
| `.bulk_group` | `.global` | `.shared::cta` | *Bulk async-group* based | *Bulk async-group* based |
...
| `.im2col::w::128` | { wHalo, wOffset } | A vector of 2 arguments containing [wHalo](#tensor-im2col-w-w128-modes-whalo) and [wOffset](#tensor-im2col-w-w128-modes-woffset) arguments. |
