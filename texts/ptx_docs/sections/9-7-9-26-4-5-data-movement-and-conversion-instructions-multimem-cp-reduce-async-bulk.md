###### 9.7.9.26.4.5. Data Movement and Conversion Instructions: `multimem.cp.reduce.async.bulk`
...
Instruction `multimem.cp.reduce.async.bulk` initiates an element-wise asynchronous reduction operation with elements from source memory address range `[srcMem, srcMem + size)` to memory locations residing on each GPUâs memory referred to by the multimem destination address range `[dstMem, dstMem + size)`.
