...
Instruction `tcgen05.mma.sp` is an asynchronous instruction which initiates an *MxNxK* matrix multiply and accumulate operation of the form `D = A*B+D` where the `A` matrix is *Mx(K/2)*, the `B` matrix is *KxN*, and the `D` matrix is *MxN*. [Sparse Matrices](#tcgen05-sparse-matrices) describes the details of the sparsity.
...
The qualifier `.cta_group::1` specifies that the matrix multiply and accumulate operation is performed on the [Tensor Memory](#tensor-memory) of the executing threadâs CTA only. The qualifier `.cta_group::2` specifies that the matrix multiply and accumulate operation is performed on the [Tensor Memory](#tensor-memory) of the executing threadâs CTA and its [peer CTA](#tcgen05-peer-cta).
...
The qualifier `.ashift` shifts the rows of the `A` matrix down by one row, except for the last row in the [Tensor Memory](#tensor-memory). Qualifier `.ashift` is only allowed with *M* = 128 or *M* = 256.
