...
The memory consistency model specification uses the terms âaddressâ or âmemory addressâ to indicate a virtual address, and the term âmemory locationâ to indicate a physical memory location.

Each PTX memory instruction also specifies the operation â either a read, a write or an atomic read-modify-write â to be performed on all the bytes in the corresponding memory location.
