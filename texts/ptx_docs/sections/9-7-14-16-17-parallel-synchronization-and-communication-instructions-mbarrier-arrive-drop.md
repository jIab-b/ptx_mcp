##### 9.7.14.16.17. Parallel Synchronization and Communication Instructions: `mbarrier.arrive_drop`
...
- Performs an [arrive-on operation](#parallel-synchronization-and-communication-instructions-mbarrier-arrive-on) on the *mbarrier object*. The operand `count` specifies the *count* argument to the [arrive-on operation](#parallel-synchronization-and-communication-instructions-mbarrier-arrive-on).
...
`mbarrier.arrive_drop` operation on an *mbarrier* object located in `.shared::cluster` but not in `.shared::cta` cannot return a value. Sink symbol â_â is mandatory for the destination operand for such cases.
