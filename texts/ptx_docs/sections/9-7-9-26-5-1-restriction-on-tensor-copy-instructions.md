...
1. `cp.reduce.async.bulk` doesnât support the types `.b4x16`, `.b4x16_p64`, `.b6x16_p32` and `.b6p2x16`.
2. `cp.async.bulk.tensor` with the direction `.global.shared::cta` doesnât support the type `.b4x16_p64`.
3. `cp.async.bulk.tensor` with the direction `.shared::cluster.global` doesnât support the sub-byte types on `sm_120a`.
4. OOB-NaN fill mode doesnât support the types `.b4x16`, `.b4x16_p64`, `.b6x16_p32` and `.b6p2x16`.
...
1. `cp.async.bulk.tensor` with the direction `.shared::cluster.global` doesnât support:
