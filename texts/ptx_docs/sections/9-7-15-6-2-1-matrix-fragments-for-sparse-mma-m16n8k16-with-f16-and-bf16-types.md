###### 9.7.15.6.2.1. Matrix Fragments for sparse `mma.m16n8k16` with `.f16` and `.bf16` types
...
- Multiplicand A: The layout of the fragments held by different threads is shown in [Figure 118](#sparse-mma-16816-f16-bf16-a). The row and column of a matrix fragment can be computed as:
  .atype
  ...
  A vector expression containing two `.b32` registers, with each register containing two non-zero `.f16` / `.bf16` elements out of 4 consecutive elements from matrix A.
