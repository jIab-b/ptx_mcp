...
Applies to all numeric types. Operands `a` and `b` have type `.type`; operands `p`, `q`, and `c` have type `.pred`. The sink symbol â_â may be used in place of any one of the destination operands.
