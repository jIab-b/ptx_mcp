...
2. If a write W precedes an *overlapping* read R in *causality order*, then for any byte accessed by both R and W, R cannot read from any write Wâ that precedes W in *coherence order*.

Litmus Test: Message Passing

|  |  |
| --- | --- |
| .global .u32 data = 0; .global .u32 flag = 0; | .global .u32 data = 0; .global .u32 flag = 0; |
| T1 | T2 |
...
| IF %r0 == 1 THEN %r1 == 1 | IF %r0 == 1 THEN %r1 == 1 |

The litmus test known as âMPâ (Message Passing) represents the essence of typical synchronization algorithms. A vast majority of useful programs can be reduced to sequenced applications of this pattern.
