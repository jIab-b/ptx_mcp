###### 9.7.15.6.2.5. Matrix Fragments for sparse `mma.m16n8k32` with `.u8` / `.s8` integer type
...
- Multiplicand A: The layout of the fragments held by different threads is shown in [Figure 128](#sparse-mma-16832-u8s8-a).
  .atype
  ...
  A vector expression containing two `.b32` registers, with each register containing four non-zero `.u8` / `.s8` elements out of 8 consecutive elements from matrix A.
