###### 9.7.15.6.2.7. Matrix Fragments for sparse `mma.m16n8k64` with `.u4` / `.s4` integer type
...
- Multiplicand A: The layout of the fragments held by different threads is shown in [Figure 138](#sparse-mma-16864-u4s4-a).
  .atype
  ...
  A vector expression containing two `.b32` registers, with each register containing eight non-zero `.u4` / `.s4` elements out of 16 consecutive elements from matrix A.
